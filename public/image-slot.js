...
(() => {
  ...
  function save() {
    try { localStorage.setItem(LS_KEY, JSON.stringify(slots)); } catch (_) {}
  }
  ...
  const stylesheet =
    ...
    '  white-space:nowrap}' +
    ':host([data-filled]:hover) .ctl,:host([data-reframe]) .ctl' +
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      this._img.addEventListener('error', () => {
        // If no user-dropped image, fall back to empty drop zone
        if (!this._userUrl) {
